...
# Data Wrangling
The daily AQI values were pulled from the Environmental Protection Agency's (EPA) AirNow dataset. The weather data was pulled from National Oceanic Atmospheric Administration (NOAA). Both datasets provided fairly complete daily data from January 1, 2010 to December 31, 2022.
...
```{r,fig.cap="Location of AQI and Weather Data Stations", echo=FALSE, message=FALSE, warning=FALSE}
data <- data.frame(
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap= "Los Angeles County Daily AQI from 2010 to 2022"}
#plotting AQI values
plot1 <- AQI_2010_2022 %>%
  ggplot(aes(x = Date,
             y = DAILY_AQI_VALUE)) +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap= "Nonseasonal Data from the Los Angeles County Daily AQI Time Series"}
#nonseasonal plot
plot2 <- ggplot(NonSeasonal.LA.AQI.daily, aes(Date, NonSeasonal)) +
  geom_point(color = "black", size = 0.5) +
  scale_y_log10() +
  geom_smooth(method = "lm", color = "red1") +
  labs(x = "Date", y = "Non-Seasonal AQI Value", 
  title = "Non-Seasonal AQI Value of Los Angeles County",
  subtitle = "2010-2022") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  finalproject.theme
